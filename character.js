...
function Character(character) {	
    ...
    const maxCourage = Math.min((baseCourage + 100) * 35, baseCourage * (Math.floor(createdCharacterCount / 10) + 1));
    let currentCourage = maxCourage;
    let currentSpeed = Math.min((baseSpeed + 5) * 10, baseSpeed + Math.floor(createdCharacterCount / 8));
    let clock = null;
